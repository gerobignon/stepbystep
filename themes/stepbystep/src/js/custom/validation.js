...
var LightValidation = function () {
		...
		formSubmitButton.addEventListener('click', function (e) {
			...
			validator.validate().then(function (status) {
				...
				if (status == 'Valid') {
					...
					$('#validation_form').request('validator::onValidate', {
						files: true,
                        success: function() {
							console.log( $('#validation_form').serializeArray(FormData));
							$('#validation_form').request('onUpdate', {
								success: function() {
					...
					'idcard': {
						validators: {
							...
							file: {
								...
								maxSize: 8388608,
								message: 'The selected file must be JPG or PNG and not exceed 8Mb.'
							}
						}
					},
					'selfie': {
	...
	return {
		...
		init: function () {
			...
			stepper = document.querySelector('#validation_stepper');
			form = stepper.querySelector('#validation_form');
